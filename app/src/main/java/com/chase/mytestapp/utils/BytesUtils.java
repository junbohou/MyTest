...
public class BytesUtils {
	...
	public static int getInt(byte[] bytes) {
		return 0xFF000000 & bytes[0] << 24 | 0xFF0000 & bytes[1] << 16 | 0xFF00
				& bytes[2] << 8 | 0xFF & bytes[3];
	}

	public static int getInt(byte[] bytes, int startIndex) {
		return 0xFF000000 & bytes[startIndex] << 24 | 0xFF0000
				& bytes[(startIndex + 1)] << 16 | 0xFF00
				& bytes[(startIndex + 2)] << 8 | 0xFF & bytes[(startIndex + 3)];
	}

	public static long getLong(byte[] bytes) {
		return 0x0 & bytes[0] << 56 | 0x0 & bytes[1] << 48 | 0x0
				& bytes[2] << 40 | 0x0 & bytes[3] << 32 | 0xFF000000
				& bytes[4] << 24 | 0xFF0000 & bytes[5] << 16 | 0xFF00
				& bytes[6] << 8 | 0xFF & bytes[7];
	}

	public static long getLong(byte[] bytes, int startIndex) {
		return 0x0 & bytes[startIndex] << 56 | 0x0
				& bytes[(startIndex + 1)] << 48 | 0x0
				& bytes[(startIndex + 2)] << 40 | 0x0
				& bytes[(startIndex + 3)] << 32 | 0xFF000000
				& bytes[(startIndex + 4)] << 24 | 0xFF0000
				& bytes[(startIndex + 5)] << 16 | 0xFF00
				& bytes[(startIndex + 6)] << 8 | 0xFF & bytes[(startIndex + 7)];
	}
	...
	public static byte[] hexStringToBytes(String hex, String subStr,
                                          boolean isLeft) {
		if ((hex == null) || ("".equals(hex))) {
			return null;
		}
	...
	public static String bytesToBinaryString(byte[] items) {
		...
		byte[] arrayOfByte = items;
		int j = items.length;
		for (int i = 0; i < j; i++) {
			byte item = arrayOfByte[i];
